...
public class TodoController {
    ...
    public ResponseEntity<CreateTodoResponse> createTodo(@RequestBody CreateTodoRequest request) {
        UUID id = commandService.createTodo(request.title(), request.description(), request.dueDate());
    ...
    public ResponseEntity<Void> updateTodo(@PathVariable UUID id, @RequestBody UpdateTodoRequest request) {
        commandService.updateTodo(id, request.title(), request.description(), request.dueDate());
        return ResponseEntity.noContent().build();
    }

    public record CreateTodoRequest(String title, String description, LocalDate dueDate) {}
    public record UpdateTodoRequest(String title, String description, LocalDate dueDate) {}
